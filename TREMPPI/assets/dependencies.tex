...
\tikzstyle{may} = [->, dashed, line width=1pt, gray]
	...
	\node[widget] (select) at (9,0) {select};
	\node[widget] (properties) at (13.5,0) {properties};
	\node[widget] (tools) at (18,0) {tools};

	\node[tool] (spawn) at (0,-2) {spawn};
	...
	\draw[must] (editor) to (spawn);
	
	
	\draw[must] (spawn) to (select);
	\draw[must] (spawn) to (properties);
	
	\draw[must] (spawn) to (bias);
	\draw[must] (spawn) to (cost);
	\draw[must] (spawn) to (express);
	\draw[must] (spawn) to (impact);
	\draw[must] (spawn) to (robustness);
	\draw[must] (spawn) to (sign);
	\draw[must] (spawn) to (trace);	
